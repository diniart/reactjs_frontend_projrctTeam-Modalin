...
const AccountForm = () => {
	...
	useEffect(() => {
		const fetchData = async () => {
			...
			let { fullname, phone, ktp, address, city, province, gender, profile_url,bank_name,bank_account_number } = project.data.data;
			setInput({ fullname, phone, ktp, address, city, province, gender, profile_url ,bank_name,bank_account_number});
		};
		fetchData();
	}, [user.token]);
					...
					<Col md={8} className="shadow ms-auto me-3" style={{ minHeight: "100vh" }}>
						<div className="flex-top">
							<div className="investee-container mt-2 p-5 text-color">
								<h3 className="mb-5">
									<i className="bi bi-building"></i> Edit Profil Saya
								</h3>
