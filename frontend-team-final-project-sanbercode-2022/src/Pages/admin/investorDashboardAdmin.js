...
const InvestorDashboardAdmin = () => {
	...
	const pageSize = 5;
	const pageCount = Math.ceil(investor.length / pageSize);
	const pages = _.range(1, pageCount + 1);

	useEffect(() => {
		const fetchData = async () => {
			const investor = await axios.get(`${apiURL}/admin/users/investor`, { headers: { Authorization: "Bearer " + user.token } });
			const result = investor.data.data;
			setInvestor(result);
			setPaginated(_(investor.data.data).slice(0).take(pageSize).value());
		};
		fetchData();
	}, [user.token]);
	...
	const pagination = (pageNo) => {
		...
		const paginated = _(investor).slice(startIndex).take(pageSize).value();
				...
				<div className="mt-3" style={{ display: "flex", flexDirection: "column", width: "75%" }}>
					<div style={{ width: "100%", height: "auto", marginBottom: "5vh" }}>
						...
						<h1 style={{ marginLeft: "3vw", marginTop: "1vw" }}>
							<i className="bi bi-wallet2"></i> Table Investor
						</h1>

						<div className="shadow" style={{ marginLeft: "3vh", marginTop: "3vw", width: "97%", backgroundColor: "white", height: "auto", paddingTop: "2vw", paddingBottom: "2vw" }}>
...
export default InvestorDashboardAdmin;
